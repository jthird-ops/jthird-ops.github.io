...
- **이미지·스타일·스크립트 파일 이름에 내용 해시가 붙습니다**
  (`style.f6bb1837.css`, `설악산.6f11f69f.jpg`).
...
- **원래 이름으로도 같은 파일을 함께 냅니다.** 방문자에게 캐시된 옛 HTML 이
  해시 없는 주소를 찾기 때문입니다. 없으면 스타일이나 그림이 빠진 화면을
  보게 됩니다. 같은 내용이면 git 이 한 번만 저장하므로 저장소가 두 배가
  되지는 않습니다(작업 폴더만 29MB → 53MB).
